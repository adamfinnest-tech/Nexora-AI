...
const AddMemoryModal = ({ isOpen, onClose, onAdd }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setFormData({ key: '', value: '', category: 'Personal', importance: 'Medium' });
    onClose();
  };
      ...
      <div className="bg-white/90 backdrop-blur-xl border border-white rounded-[2rem] w-full max-w-md shadow-2xl overflow-hidden animate-slide-up">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input required type="text" placeholder="e.g. Name, Company, Pet" value={formData.key} onChange={e => setFormData({...formData, key: e.target.value})} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#A573FF]/50 bg-white/50 font-medium text-gray-800" />
          </div>
          <div>
            <label className="block text-[13px] font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Value</label>
            <textarea required rows={2} placeholder="e.g. John Doe, Amazon, Buddy" value={formData.value} onChange={e => setFormData({...formData, value: e.target.value})} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#A573FF]/50 bg-white/50 resize-none font-medium text-gray-800" />
          </div>
